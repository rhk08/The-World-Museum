...
const TimelinePage = () => {
  ...
  const animateAndScrollDivs = () => {
    const divs = Array.from(container.current.querySelectorAll(']'));
    const smoother = ScrollSmoother.get();
    ...
    sortedDivs.forEach((div, index) => {
      ...
      gsap.from(div, {
        ...
        onStart: () => {
          if (smoother) {
            const offset = -(window.innerHeight / 2 - div.offsetHeight / 2);
            smoother.scrollTo(div, { duration: 0.8, offsetY: offset });
          }
        },
      });
    });
